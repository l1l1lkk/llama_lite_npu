...
def pytorch_flash_decode_stage2(mid_o, mid_o_logexpsum, actual_seq_len, partition_size):
    ...
    atten_output_pt = torch.zeros(
        batchs, num_heads, head_dim, device="npu:6", dtype=torch.float32
    )

    for batch in range(batchs):
        for head in range(num_heads):
            d_i = torch.zeros(head_dim, device="npu:6", dtype=torch.float32)
            m_i = torch.full(
                (head_dim,), -float("inf"), device="npu:6", dtype=torch.float32
            )  # 初始化为 [head_dim]
            acc = torch.zeros(head_dim, device="npu:6", dtype=torch.float32)
            for partition in range(seq_block_num):
                ...
                part_max_tensor = torch.full(
                    (head_dim,), part_max, device="npu:6", dtype=torch.float32
                )
...
mid_o = torch.randn(
    batchs, num_heads, seq_block_num, head_dim, device="npu:6", dtype=torch.float32
)
mid_o_logexpsum = torch.randn(
    batchs, num_heads, seq_block_num, device="npu:6", dtype=torch.float32
)
