...
def bench_mlp_silu(M, N, provider, mode="forward", eps=1e-5, device="npu:6"):
    """定义性能测试函数 bench_softmax。
    参数：
        M: 批量大小（固定为 4096)。
        N: 特征维度大小（作为 x 轴变量变化）。
    ...
    x = torch.randn(B, N, hidden_size, device="npu:6", dtype=torch.float16)
    w1 = (
        torch.randn(
            (intermediate_size, hidden_size), device="npu:6", dtype=torch.float16
        )
        * 0.01
    )
    w2 = (
    ...
    w3 = (
        torch.randn(
            (hidden_size, intermediate_size), device="npu:6", dtype=torch.float16
        )
        * 0.01
    )
...
def bench_flash_attention(
    BATCH, H, N_CTX, HEAD_DIM, causal, mode, provider, device="npu:6"
):
    assert mode in ["fwd"]
    dtype = torch.float16
...
def benchmark(SEQ_LEN, HIDDEN_SIZE, BATCH_SIZE, HEAD_NUM, provider):
    ...
    rotary_pos_emb = LlamaRotaryEmbedding(HIDDEN_SIZE, device="npu:6")
