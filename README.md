...
![Version](https://img.shields.io/badge/version-0.0.8rc3-blue)
...
Current version: **0.0.8rc3** (2026-06-22)
...
`v0.0.8rc3` fixes TP chunked-prefill capacity preflight so worker ranks no longer discover KV allocation failures before rank 0 scheduler handling. Exact Prefix Cache remains enabled by default; page-aligned partial Prefix Cache remains explicit via `--partial_prefix_cache`.
...
`v0.0.8rc3` has not been re-benchmarked on Atlas yet. The table below keeps the latest reproducible measured baselines and is not a new-version performance claim:
...
- Q+KV、Gate+Up 尚未融合；
- v0.0.8rc3 Prefix Cache defaults to exact repeated greedy prompts only; page-aligned partial prefix reuse is opt-in through `--partial_prefix_cache`;
