...
![Version](https://img.shields.io/badge/version-0.0.8rc5-blue)
...
Current version: **0.0.8rc5** (2026-06-22)
...
`v0.0.8rc5` makes TP continuous-batching command dispatch transactional: rank 0 sends decode state snapshots, worker ranks acknowledge command execution, and state mismatches are reported before entering model collectives. Exact Prefix Cache remains enabled by default; page-aligned partial Prefix Cache remains explicit via `--partial_prefix_cache`.
...
`v0.0.8rc5` has not been re-benchmarked on Atlas yet. The table below keeps the latest reproducible measured baselines and is not a new-version performance claim:
...
- Q+KV、Gate+Up 尚未融合；
- v0.0.8rc5 Prefix Cache defaults to exact repeated greedy prompts only; page-aligned partial prefix reuse is opt-in through `--partial_prefix_cache`;
