...
def test_rmsnorm(batch_size, N, hidden_size):
    x = torch.randn(batch_size, N, hidden_size, device="npu:6", dtype=torch.float16)
    w = torch.randn(hidden_size, device="npu:6", dtype=torch.float16)
...
def test_skip_rmsnorm(batch_size, N, hidden_size):
    x = torch.randn(batch_size, N, hidden_size, device="npu:6", dtype=torch.float16)
    r = torch.randn(batch_size, N, hidden_size, device="npu:6", dtype=torch.float16)
    w = torch.randn(hidden_size, device="npu:6", dtype=torch.float16)
...
def benchmark_skip_rmsnorm(batch_size, N, iters=1000):
    x = torch.randn(batch_size, N, device="npu:6", dtype=torch.float16)
    r = torch.randn(batch_size, N, device="npu:6", dtype=torch.float16)
    w = torch.randn(N, device="npu:6", dtype=torch.float16)
...
def benchmark(func, shapes, warmup=10, iters=50):
    times = []
    for shape in shapes:
        X = torch.randn(shape, dtype=torch.float16, device="npu:6")
        R = torch.randn(shape, device="npu:6", dtype=torch.float16)
        W = torch.randn(shape[-1], dtype=torch.float16, device="npu:6")
        # warmup
