...
def test():
    ...
    q = torch.empty(
        (Z * (N_CTX - prompt_cache_len), H, D_HEAD), dtype=dtype, device="npu:6"
    ).normal_(mean=0.1, std=0.2)
    k = torch.empty((Z * N_CTX, H, D_HEAD), dtype=dtype, device="npu:6").normal_(
        mean=0.4, std=0.2
    )
    v = torch.empty((Z * N_CTX, H, D_HEAD), dtype=dtype, device="npu:6").normal_(
        mean=0.3, std=0.2
    )
    o = torch.empty(
        (Z * (N_CTX - prompt_cache_len), H, D_HEAD), dtype=dtype, device="npu:6"
    ...
    req_to_token_indexs = torch.empty(
        (1000, N_CTX + 7000), dtype=torch.int32, device="npu:6"
    )
    max_input_len = N_CTX
    b_start_loc = torch.zeros((Z,), dtype=torch.int32, device="npu:6")
    b_seq_len = torch.ones((Z,), dtype=torch.int32, device="npu:6")
    b_req_idx = torch.ones((Z,), dtype=torch.int32, device="npu:6")
    b_prompt_cache_len = torch.zeros(Z, dtype=torch.int32, device="npu:6")
...
def test2():
    ...
    kv = torch.empty((Z, 2 * H, N_CTX, D_HEAD), dtype=dtype, device="npu:6").normal_(
        mean=0.4, std=0.2
    )
    k = kv[:, :H]
    v = kv[:, H:]
    # v = torch.empty((Z, H, N_CTX, D_HEAD), dtype=dtype, device="npu:6").normal_(mean=0.3, std=0.2)
    o = torch.empty(
        (Z * (N_CTX - prompt_cache_len), H, D_HEAD), dtype=dtype, device="npu:6"
    ).normal_(mean=0.3, std=0.2)
    torch_o = torch.empty(
