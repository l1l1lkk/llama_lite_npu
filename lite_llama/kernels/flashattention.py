...
def test_prefill_stage():
    ...
    torch.manual_seed(0)
    q = torch.randn(
        batch_size, num_heads, seq_length, head_dim, device="npu:6", dtype=torch.float32
    )
    k = torch.randn(
        batch_size, num_heads, seq_length, head_dim, device="npu:6", dtype=torch.float32
    )
...
def test_decode_stage():
    ...
    o_initial = torch.zeros_like(q_initial, device="npu:6", dtype=torch.float32)
    new_token_q = torch.randn(
        batch_size, num_heads, 1, head_dim, device="npu:6", dtype=torch.float32
    )
