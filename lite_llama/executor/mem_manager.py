...
class ComputeMaxAvailableBlocks:
    ...
    def compute_num_available_blocks(self, model, model_path=None):
        ...
        free_memory_pre_profile, total_gpu_memory = torch.npu.mem_get_info()
        ...
        logger.info(f"模型加载后可用内存: {torch.npu.mem_get_info()[0] / (1024**3):.2f} GB")
        # 同步 CUDA 操作，确保内存信息准确
        torch.npu.synchronize()
        # 计算模型加载后的峰值内存使用量. Get the peak memory allocation recorded by torch
        peak_memory = torch.npu.memory_stats()["allocated_bytes.all.peak"]
        # 清理未使用的缓存，计算非 Torch 分配的内存. 检查是否有任何剩余内存可能已在“torch”之外的 gpu 上分配。例如，NCCL 操作在前向传递期间可能会使用几 GB
        torch.npu.empty_cache()
        torch_allocated_bytes = torch.npu.memory_stats()["allocated_bytes.all.current"]
        
        total_allocated_bytes = torch.npu.mem_get_info()[1] - torch.npu.mem_get_info()[0]
        ...
        torch.npu.empty_cache() # 再次清理 CUDA 缓存
