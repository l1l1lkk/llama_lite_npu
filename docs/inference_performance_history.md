...
## 2026-07-03 v0.0.11rc1 / v0.0.12rc1 Planned Measurement Matrix

???? 0.0.11 ? 0.0.12 ???????????????????????

### ????

- v0.0.11rc1??? vocab-parallel Top-P ??????? Top-P ??????????
- v0.0.12rc1???? Chunked Prefill??/? prompt ?? packed prefill?? prompt ????

### ???????

| ?? | ?? | ???? |
|---|---|---|
| Greedy p1 fixed | ?? decode ????? | output throughput?TTFT?ITL |
| Top-P p4 fixed | ???? Top-P candidate path | output throughput??? fallback ?? |
| Chunked adaptive p4 2048 | ??? prompt ??? | TTFT?queue wait?prefill ?? |
...
?? `fallback_rows_total / candidate_rows_total` ???????? `--sampling_candidate_k` ??????????? Top-P ???
