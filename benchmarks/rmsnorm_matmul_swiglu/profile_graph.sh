#!/usr/bin/env bash
# Capture matched fused Triton decode traces with NPU Graph off/on.

set -eo pipefail

CHECKOUT=${CHECKOUT:-/data/liuke/llama_lite_npu-worktrees/matmul-swiglu-rmsnorm-fused-optimization}
RESULT_ROOT=${1:-/data/liuke/rmsnorm_matmul_swiglu_20260730/e2e/profiler}
PORT=${PORT:-8017}
MASTER_PORT=${MASTER_PORT:-29617}
PROFILE_DURATION=${PROFILE_DURATION:-8}
HARNESS="$CHECKOUT/benchmarks/swiglu"
...
cleanup() {
  if [[ -n "$active_server_root" ]]; then
    bash "$HARNESS/stop_e2e_server.sh" "$active_server_root" "$PORT" || true
  fi
}
trap cleanup EXIT
...
run_request() {
  local output=$1
  local requests=$2
  python "$HARNESS/e2e_client.py" \
    --base-url "http://127.0.0.1:$PORT" \
    --workload "$WORKLOAD" \
    --checkout "$CHECKOUT" \
    --server-branch feature/matmul-swiglu-rmsnorm-fused-optimization \
...
for graph_mode in off on; do
  ...
  PROFILING_MODE=dynamic \
    bash "$HARNESS/start_e2e_server.sh" \
  ...
  bash "$HARNESS/stop_e2e_server.sh" "$server_root" "$PORT"
  active_server_root=
done
