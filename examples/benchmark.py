...
def transformers_inference(
    ...
    print("input device:", model_inputs.input_ids.device, flush=True)
...
def compare_inference_speed(
    ...
    """
    ckpt_dir_lower = lite_llama_ckpt_dir.lower()

    if "qwen3" in ckpt_dir_lower:
        model_type = "qwen3"
    elif "qwen2" in ckpt_dir_lower:
        model_type = "qwen2"
    elif "llava" in ckpt_dir_lower:
        model_type = "llava"
    elif "llama" in ckpt_dir_lower:
        model_type = "llama"
    else:
        raise ValueError(f"Unsupported model type for checkpoint dir: {lite_llama_ckpt_dir}")
    ...
    if getattr(device, "type", str(device)) == "npu" or str(device).startswith("npu"):
        torch.npu.empty_cache()
    elif getattr(device, "type", str(device)) == "cuda" or str(device).startswith("cuda"):
...
def main():
    ...
    # 根据实际情况修改
    hf_model_name = "/data/zcg/models_weight/Qwen3-1.7B"
    custom_checkpoints_dir = "/data/zcg/lite_llama-main/my_weight/Qwen3-1.7B"
